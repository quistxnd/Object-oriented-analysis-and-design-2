...
pygame.display.set_caption("Card Game: WITHOUT PATTERNS (Bad Code)")
...
class Card:
    def __init__(self, name, cost, color, desc, card_type):
        self.name = name
        self.cost = cost
        self.color = color
        self.desc = desc
        self.card_type = card_type  # Храним тип строкой

        self.pos = pygame.Vector2(WIDTH // 2, HEIGHT + 300)
    ...
    def play_card(self, owner, opponent, effects):
        ...
        if self.card_type == "STRIKE":
            effects.append(Projectile(owner.pos, opponent.pos, self.color, lambda: opponent.take_damage(8)))

        elif self.card_type == "SHIELD":
            owner.armor += 10
            effects.append(FloatingText(owner.pos, "+10 БРОНЯ", self.color))
            for _ in range(15): effects.append(Particle(owner.pos, self.color))

        elif self.card_type == "METEOR":
            effects.append(Projectile(owner.pos, opponent.pos, self.color, lambda: opponent.take_damage(25), size=16))

        elif self.card_type == "HEAL":
            owner.take_heal(12)
            effects.append(FloatingText(owner.pos, "+12 HP", self.color))
            for _ in range(25): effects.append(Particle(owner.pos, self.color))

        elif self.card_type == "FOCUS":
            owner.mana = min(owner.mana + 3, 10)
            effects.append(FloatingText(owner.pos, "+3 МАНА", self.color))
            for _ in range(15): effects.append(Particle(owner.pos, self.color))
...
class Engine:
    def __init__(self):
        ...
        # Вместо классов мы передаем лямбда-функции (фабрики), создающие один и тот же класс Card
        self.pool = [
            lambda: Card("УДАР", 1, CLR['phys'], "Урон: 8", "STRIKE"),
            lambda: Card("УДАР", 1, CLR['phys'], "Урон: 8", "STRIKE"),
            lambda: Card("СТАЛЬНОЙ ПАНЦИРЬ", 1, CLR['armor'], "Броня: 10\nСпадает в начале хода", "SHIELD"),
            lambda: Card("ИСТОЧНИК", 2, CLR['heal'], "Хил: 12", "HEAL"),
            lambda: Card("МЕДИТАЦИЯ", 0, CLR['mana'], "Мана: +3", "FOCUS"),
            lambda: Card("МЕТЕОРИТ", 4, CLR['fire'], "Сокрушительный\nурон: 25", "METEOR")
        ]
    ...
    def draw_card(self, card):
        ...
        draw_icon(surf, card.card_type, card.color, CARD_W // 2, CARD_H // 2 - 15)
    ...
    def draw_game_over(self):
        ...
        if self.win:
            ...
            story = [
                ...
                "Но боевой дух юного программиста (хоть и без паттернов)",
            ...
            story = [
                ...
                "А студент ответил: «У вас же спагетти-код в if-elif!»",
                "и безжалостно отправил преподавателя на пересдачу.",
    ...
    def run(self):
        while True:
            ...
            if self.state == "MENU":
                self.draw_bg()
                title = FONT_L.render("ANTI-PATTERNS & STUDENTS", True, CLR['mana'])
                SCREEN.blit(title, (WIDTH // 2 - title.get_width() // 2, 120))
